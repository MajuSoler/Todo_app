  ...
  let getRemixManifestForDev = async () => {
    ...
    for (let [key, route] of Object.entries(ctx.remixConfig.routes)) {
      ...
      routes[key] = {
        ...
        index: route.index,
        caseSensitive: route.caseSensitive,
        module: path__namespace.posix.join(ctx.remixConfig.publicPath, `${resolveFileUrl.resolveFileUrl(ctx, resolveRelativeRouteFilePath(route, ctx.remixConfig))}${CLIENT_ROUTE_QUERY_STRING}`),
        hasAction: sourceExports.includes("action"),
        hasLoader: sourceExports.includes("loader"),
        hasClientAction: sourceExports.includes("clientAction"),
        hasClientLoader: sourceExports.includes("clientLoader"),
    ...
    config: async (_viteUserConfig, _viteConfigEnv) => {
      ...
      return {
        ...
        ssr: {
          ...
          "@remix-run/architect", "@remix-run/cloudflare-pages", "@remix-run/cloudflare-workers", "@remix-run/cloudflare", "@remix-run/css-bundle", "@remix-run/deno", "@remix-run/dev", "@remix-run/express", "@remix-run/netlify", "@remix-run/node", "@remix-run/react", "@remix-run/serve", "@remix-run/server-runtime"]
        },
        optimizeDeps: {
          include: [
          ...
          build: {
            ...
            ...(!viteConfigEnv.isSsrBuild ? {
              ...
              rollupOptions: {
                ...
                input: [ctx.entryClientFilePath, ...Object.values(ctx.remixConfig.routes).map(route => `${path__namespace.resolve(ctx.remixConfig.appDirectory, route.file)}${CLIENT_ROUTE_QUERY_STRING}`)]
              }
            } : {
